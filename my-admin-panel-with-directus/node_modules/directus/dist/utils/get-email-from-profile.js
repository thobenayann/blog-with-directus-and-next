...
function getEmailFromProfile(provider, profile) {
    ...
    const email = (0, lodash_1.get)(profile, path);
    if (!email) {
        throw new exceptions_1.ServiceUnavailableException(`Email address not found. Profile "${JSON.stringify(profile)}", path: "${path}"`, {
            service: 'oauth',
            provider,
            profile,
            path,

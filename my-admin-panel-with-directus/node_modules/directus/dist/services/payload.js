...
const geometry_2 = require("../database/helpers/geometry");
...
class PayloadService {
    ...
    async processValues(action, payload) {
        let processedPayload = (0, utils_1.toArray)(payload);
        ...
        processedPayload = processedPayload.map((item) => (0, flat_1.unflatten)(item, { delimiter: '->' }));
    ...
    processGeometries(payloads, action) {
        const helper = (0, geometry_2.getGeometryHelper)();
        const process = action == 'read'
            ? (value) => {
                if (typeof value === 'string')
                    return (0, wellknown_1.parse)(value);
            }
            : (value) => helper.fromGeoJSON(typeof value == 'string' ? JSON.parse(value) : value);
        const fieldsInCollection = Object.entries(this.schema.collections[this.collection].fields);
        const geometryColumns = fieldsInCollection.filter(([_, field]) => (0, geometry_1.isNativeGeometry)(field));
